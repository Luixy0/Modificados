function  [xbest,fobj,X,fit] = fuzzyL2(F_index)
...
if dim<=2 
figure(1) %Gráfica de la población inicial
hold on
                            x=[low:rango/100:up];
                            y=x;
                            [xg,yg]=meshgrid(x,y);
                            ...
                            for l=1:col
                                for h=1:row
                                    z(h,l)=evaluateF([xg(h,l),yg(h,l)],F_index);
...
%     fmax=Lightn(end);
    ...
    for j=1:Np
      ...
      R = tsugeno(beta,beta1,Per,dim,xbest,X(j,:),up,low,fobj,fmax,evaluateF(X(j,:) ,F_index));
     
      for k = 1:dim
        Temp(1,k) = ((w1 * R(k,1) + w2 * R(k,2) + w3 * R(k,3) + w4 * R(k,4)) / (w1+w2+w3+w4))*Tun;
        %X(j,k) = X(j,k) + (Temp(1,k));
        X2(1,k) = X(j,k) + (Temp(1,k));
      end
      fitnessAUX_2=evaluateF(X2(1,:) ,F_index);
	  i = i+1;
	  % eliminar evaluateF(X(j,:)
      fitnessAUX = evaluateF(X(j,:),F_index);
...
%                             zz=f(X1,Y1);
                            ...
                            for q=1:col1
                                for hh=1:row1
                                    zz(hh,q)=evaluateF([X1(hh,q),Y1(hh,q)],F_index);
